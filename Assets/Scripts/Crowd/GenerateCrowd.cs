...
public class GenerateCrowd : MonoBehaviour
{
    ...
    //Script that generates a crowd of a random size each time the game is played
    void Start()
    {
        GameObject[] spawns = GameObject.FindGameObjectsWithTag("Spawn");
        foreach (GameObject spawnPoint in spawns)
        {
            //30% chance of spawning random crowd member in seat, quaternion.identity as no rotation is needed  
